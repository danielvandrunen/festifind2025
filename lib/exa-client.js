...
async function performComprehensiveResearch(festivalName) {
  ...
  // PHASE 2: Targeted Research Based on Identified Companies
  const targetedSearchQueries = [
    // LinkedIn stakeholder research - EMPLOYMENT-FOCUSED SEARCHES
    ...identifiedCompanies.map(company => ({
      query: `site:linkedin.com/in ("works at ${company.name}" OR "employee at ${company.name}" OR "director at ${company.name}" OR "CEO at ${company.name}" OR "founder of ${company.name}" OR "manager at ${company.name}")`,
    ...
    // News articles for business intelligence
    {
      query: `"${festivalName}" attendance OR capacity OR "tickets sold" OR revenue news OR press`,
      category: 'news',
  ...
  console.log(`Phase 1: ${phase1Results.length} strategies, Phase 2: ${phase2Results.length} strategies`);
