...
/**
 * Main scraper function for EB Live - this is a server-side only implementation
 */
async function scrapeEBLive(options = {}) {
  const {
    maxPages = 41, // Default to all 41 pages
    delay = 1500,
    outputDir = dataDir,
    testMode = false,
    onProgressUpdate = () => {},
  ...
  const log = (message, level = 'info') => {
    console.log(`[EBLive] ${level}: ${message}`);
  ...
  const browserOptions = {
    headless: true,
    executablePath: dockerMode ? '/usr/bin/chromium' : undefined, // Use system Chromium in Docker
    args: [
      '--disable-gpu',
...
export default async function handler(req, res) {
  ...
  // Prevent multiple scraper instances from running at once
  if (isScraperRunning) {
    return res.status(429).json({ 
      success: false, 
      message: 'A scraper is already running. Please wait for it to complete.'
    });
  }

  try {
    // Capture logs and progress for the UI
    let logs = [];
    let progress = 0;
    
    const onProgressUpdate = (value) => {
      progress = value;
    };
    
    const onLogUpdate = (log) => {
      logs.push(log);
      // Keep log size reasonable by removing older entries
      if (logs.length > 1000) {
        logs = logs.slice(-500); // Keep the latest 500 logs
      }
    };
    
    // Capture request parameters
    const maxPages = req.body?.maxPages || 41; // Default to all pages (41)
    const testMode = req.body?.testMode === true;
    const dockerMode = process.env.DOCKER === 'true' || process.env.CONTAINER === 'true';
    ...
    // Now run the actual scraper (this happens after response is sent)
    try {
      isScraperRunning = true;
      
      // Write initial status file so the UI can show progress
      const initialStatusPath = path.join(dataDir, 'latest-status.json');
      fs.writeFileSync(initialStatusPath, JSON.stringify({
        success: true,
        inProgress: true,
        timestamp: new Date().toISOString(),
      ...
      const { festivals, metrics } = await scrapeEBLive({
        maxPages, 
        outputDir: dataDir,
        onProgressUpdate: (value) => {
          progress = value;
          
          // Update status file with current progress every 5%
          if (value % 5 === 0 || value === 100) {
            try {
              fs.writeFileSync(initialStatusPath, JSON.stringify({
                ...
                lastRun: new Date().toISOString(),
                progress: value,
                message: `Scraping in progress: ${value}% complete`
              }));
            } catch (err) {
      ...
      // Save results to a file that can be fetched later
      const resultsPath = path.join(dataDir, `results-${new Date().toISOString().replace(/:/g, '-')}.json`);
      fs.writeFileSync(resultsPath, JSON.stringify({
        success: true,
        timestamp: new Date().toISOString(),
        festivalCount: metrics.uniqueFestivals,
        pages: metrics.pages,
        timeElapsed: metrics.timeElapsed,
        metrics,
        festivals: festivals.slice(0, 100) // Limit to first 100 festivals for UI display
      }, null, 2));
      
      // Save status file for the UI to check
      ...
      fs.writeFileSync(statusPath, JSON.stringify({
        ...
        timestamp: new Date().toISOString(),
        lastRun: new Date().toISOString(),
        error: scrapingError.message || 'Unknown error during scraping',
        progress: progress
      }));
    } finally {
    ...
    // This error handler is unlikely to be reached since we already sent a response
    isScraperRunning = false;
  }
} 